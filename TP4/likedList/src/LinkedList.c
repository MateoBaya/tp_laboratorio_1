...
static Node* getNode(LinkedList* this, int nodeIndex)
{
	Node * nodoObtenido=NULL;
	if(this != NULL && nodeIndex >= 0 && nodeIndex < ll_len(this))
	{
		nodoObtenido=this->pFirstNode;
		for(int i=0;i<nodeIndex;i++)
		{
			nodoObtenido=nodoObtenido->pNextNode;
		}
	}
    return nodoObtenido;
}
...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
	...
	Node * nodoAux=NULL;
	if(this!=NULL&&nodo!=NULL&&nodeIndex>=0&&nodeIndex<=ll_len(this))
	{
    ...
    if(this!=NULL&&index>=0&&index<ll_len(this))
    {
		...
		if(nodoABorrar!=NULL)
		{
			if(index)
			{
				...
				if(nodoAux!=NULL)
				{
					nodoAux->pNextNode=nodoABorrar->pNextNode;
				}
				returnAux=0;
			}
			else
    ...
    if(this!=NULL&&from>=0&&to>=from&&to<=ll_len(this))
    {
    	...
    	if(cloneArray!=NULL)
    	{
    		for(int i=from;i<=to;i++)
    		{
    			...
    			if(pElement!=NULL&&ll_add(cloneArray, pElement))
    			{
    ...
    if(this!=NULL&&(order==0||order==1)&&pFunc!=NULL)
    {
    	for(int i=0;i<ll_len(this)-1;i++)
    	{
    		for(int j=i+1;j<ll_len(this);j++)
    		{
    			...
    			if(((*pFunc)(elementoA,elementoB)==1&&order)||((*pFunc)(elementoA,elementoB)==-1&&!order))
    			{
